...
**状态**：代码就位并验证；**根因已定位**（见第三节）。engine 形态在 **Metal RHI** 下
11/11 全绿通过（文件 14），在 **Vulkan/MoltenVK** 下 **4/4 失败**。
正式 30 min 长跑的后端选择待定（见文末）。
...
engine 形态的正式 30 min 长跑用哪条路，需要一次方向决策：

- **A｜用 Metal RHI 跑 T13**：立即可得合流形态 30 min 稳定性数据；把"Vulkan/MoltenVK
  后端不可用"作为独立缺陷记录（含本目录的最小复现），供后续报 Qt/MoltenVK 或架构复议。
- **B｜继续攻 Vulkan 路径**：尝试把引擎重 GL 初始化前移到 QML 初始化之前（治标，
  不保证 30 min 长跑不炸），或深入 MoltenVK 配置项。
- **C｜先 B 快速试一次，不成立刻转 A**。
