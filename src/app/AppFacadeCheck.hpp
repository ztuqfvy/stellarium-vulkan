 ...
 *
 * ── T17 新增（QML 键盘挂载点）────────────────────────────────────────────
 *   AC-12 挂载点端到端：QML 里"接 routeKey 的那个 Item"必须存在且确实是 Item
 *        （Keys 只能附加在 Item 上）；若场景图有 activeFocusItem，则把 Q 键
 *        **真的从窗口投递**，断言引擎动作恰触发一次。
 *        这条补的是 AC-5 的盲区——AC-5 直接调 routeKey，证明不了 QML 接上了没有；
 *        而 T15 的挂载点从未生效（Keys 挂在了 ApplicationWindow 上）。
 *        窗口未激活时 SKIP 并写明"端到端未验"，不伪装成 OK。
 *
 * ── T16 新增（单一仿真时钟）───────────────────────────────────────────────
