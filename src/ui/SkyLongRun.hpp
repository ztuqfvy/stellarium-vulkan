 ...
 *       → p99 改相对口径 ≤ min(3×帧间隔, 100ms)；
 *       max 改**占比口径**（T13 正式跑首战定案）：> min(6×帧间隔, 200ms) 的帧
 *       占比 ≤0.01%。原因：max 统计在 5 万样本下对单个调度毛刺零容忍——
 *       首战 30min 稳态窗 48846 帧仅 1 帧 168.33ms（0.002%，系统日志无外因）
 *       就判死整跑；真卡顿另有判据管（degraded → SL-C10，持续卡顿按占比自然
 *       超限）。负控仍有效：150ms/s 注入 ≈2.4% 超限帧 >> 0.01%。
